...
/**
 * Coordinates game flow while delegating rules and mutations to the existing
...
public class GameEngine {
    ...
    public GameEngine(Board board, List<Player> players,
                      RuleEngine ruleEngine, CaptureHandler captureHandler,
                      BlockHandler blockHandler, MysteryManager mysteryManager,
                      IDice dice, ICoinToss coinToss) {
        this.board = board;
        ...
        this.landingResolver = new LandingResolver(
    ...
    public int getRoundCount() { return turnManager.getRoundCount(); }
    ...
    private boolean executeBaseEntry(Player player, List<Piece> validMoves,
                                     int diceValue) {
        ...
        CommandResult result = executeCommand(new EnterBoardCommand(
                piece, board, coinToss, landingResolver));

        events.pieceEnteredBoard(player.getColor(), piece.getFullName(),
                player.getPiecesOnBoard().size(), player.getPiecesInBase().size());
    ...
    private boolean executeMove(Player player, Piece piece, int diceValue) {
        if (piece.isInHomeStraight()) {
            ...
            CommandResult result = executeCommand(new HomeMoveCommand(
        ...
        if (blockPosition != -1) {
            return executePartialBlockedMove(
                    player, piece, diceValue, fromPosition, blockPosition);
        }

        int destination = ruleEngine.calculateDestination(piece, diceValue);
        CommandResult result = executeCommand(new MoveCommand(
                piece, board, destination, effective, landingResolver));
        publishMovement(player, piece, result);
        publishLandingEvents(player, piece, result);
        return result.hasCaptured();
    }

    private boolean tryEnterHomeStraight(Player player, Piece piece,
                                         int diceValue, int effective) {
        ...
        CommandResult result = executeCommand(new HomeMoveCommand(
                piece, board, destinationIndex, effective));
        publishMovement(player, piece, result);
        return true;
    }
    ...
    private boolean executeBlockMove(Player player, Piece chosen, int diceValue) {
        ...
        CommandResult result = executeCommand(new BlockMoveCommand(
                block, diceValue, board, blockHandler,
                captureHandler, landingResolver));
    ...
    private boolean executePartialBlockedMove(Player player, Piece piece,
                                              int diceValue, int fromPosition,
                                              int blockPosition) {
        Cell blockingCell = board.getCellAt(blockPosition);
        String blockingColor = blockingCell.hasPieces()
                ? blockingCell.getPieces().getFirst().getColor() : "";
        String blockingName = blockingCell.hasPieces()
                ? blockingCell.getPieces().getFirst().getFullName() : "";
        events.pieceBlocked(player.getColor(), piece.getFullName(), fromPosition,
                blockPosition, blockingColor, blockingName);
        ...
        CommandResult result = executeCommand(new MoveCommand(
                piece, board, stopPosition, steps, landingResolver));
    ...
    private void publishMovement(Player player, Piece piece, CommandResult result) {
        ...
        events.pieceMoved(player.getColor(), piece.getFullName(),
                result.getFromPosition(), result.getToPosition(),
                result.getMovement(), result.getDirection());
    }

    private void publishLandingEvents(Player player, Piece actor,
        ...
        for (Piece captured : result.getCapturedPieces()) {
            ...
            events.pieceCaptured(player.getColor(), actor.getFullName(),
                    result.getCapturePosition(captured), captured.getColor(),
                    captured.getFullName(), boardCount, baseCount);
        }
        if (result.getMysteryOutcome() != null) {
            events.mysteryResolved(player.getColor(), actor.getFullName(),
                    result.getMysteryOutcome());
        }
    }
    ...
    private void fireBlockedResult(Player player, Piece piece, CommandResult result) {
        ...
        String color = blockingCell.hasPieces()
                ? blockingCell.getPieces().getFirst().getColor() : "";
        String name = blockingCell.hasPieces()
                ? blockingCell.getPieces().getFirst().getFullName() : "";
        events.pieceBlocked(player.getColor(), piece.getFullName(), piece.getPosition(),
                result.getBlockedAt(), color, name);
        events.noOtherPieces(player.getColor());
    }
