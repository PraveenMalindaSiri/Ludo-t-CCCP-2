...
class MysteryManagerTest {
    ...
    void setUp() {
        ...
        effect = (piece, gameBoard) -> new MysteryOutcome(MysteryOutcome.Type.START, "test");
    }
    ...
    void managerRequiresAtLeastOneEffect() {
        assertThrows(
                ...
                () -> new MysteryManager(board, new TestSupport.SequenceRandom(0), List.of()));
    }
    ...
    void spawnAvoidsOccupiedCells() {
        ...
        MysteryManager manager =
                new MysteryManager(board, new TestSupport.SequenceRandom(0), List.of(effect));
        ...
        assertEquals(
                GameConfig.getInstance().getMysteryCellDuration(), manager.getRoundsRemaining());
    }

    @Test
    void inactiveManagerSpawnsAfterConfiguredRoundsWithPiecesOnPath() {
        MysteryManager manager =
                new MysteryManager(board, new TestSupport.SequenceRandom(6), List.of(effect));
    ...
    void expiredMysteryRespawnsAtDifferentPosition() {
        MysteryManager manager =
                new MysteryManager(board, new TestSupport.SequenceRandom(5, 5), List.of(effect));
        manager.spawnMysteryCell();
